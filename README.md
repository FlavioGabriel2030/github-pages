# Meu Portfólio Profissional - Flavio Gabriel
Este repositório foi desenvolvido como parte de um desafio acadêmico para consolidar projetos pessoais e acadêmicos, aplicando as melhores práticas de versionamento com Git e GitHub. O objetivo é apresentar minhas competências técnicas em Desenvolvimento de Software e Manutenção de Hardware de forma estruturada e profissional

🚀 Sobre Mim
Sou estudante de Ciência da Computação e técnico profissional especializado em manutenção, formatação e otimização de hardware. Atualmente, foco meus estudos em Python, Java e SQL, além de gerenciar projetos de otimização de sistemas Windows.

📁 Estrutura do Repositório
Para manter a organização, o repositório está dividido nas seguintes seções:  
/academicos: Projetos desenvolvidos durante a graduação.  
/pessoais: Scripts de automação, otimizadores e projetos de interesse pessoal.  
/documentacao: Slides, PDFs e materiais de apoio das atividades.  

💻 Projetos em Destaque

1. Website Pessoal (Versão 1.0)
Descrição: Landing page criada para este desafio, servindo como base para o GitHub Pages.
Tecnologias: HTML5, CSS3.Status: v1.0 finalizada e enviada ao repositório remoto.

2. Otimizador de Windows (Batch Script)
Descrição: Script customizado para limpeza e ganho de performance em sistemas Windows, focado em manutenção profissional.
Tecnologias: Shell/Batch.

🌐 GitHub Pages e Apresentação
Link do Portfólio (Web): https://flaviogabriel2030.github.io/github-pages/.
Vídeo de Apresentação (YouTube): [Insira aqui o link do seu vídeo de 5 minutos].

🤝 Contato e Conectividade
O conteúdo deste repositório está integrado ao meu perfil profissional no LinkedIn para facilitar o compartilhamento com recrutadores e parceiros da área de TI.
LinkedIn: [Insira o link do seu perfil aqui]
E-mail: [Seu e-mail aqui]
